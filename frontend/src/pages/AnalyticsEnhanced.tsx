...
import { exportToJSON, exportToCSV, copyToClipboard } from '@/lib/exportUtils';
import {
    BarChart,
    Bar,
    XAxis,
...
export function AnalyticsEnhanced() {
    ...
    const handleExportJSON = () => {
        ...
        toast({ title: "Exported!", description: "Simulation data exported as JSON." });
    };

    const handleExportCSV = () => {
        if (!analysisResults) return;
        exportToCSV(analysisResults);
        toast({ title: "Exported!", description: "Simulation data exported as CSV." });
    };

    const handleShare = async () => {
        const success = await copyToClipboard(window.location.href);
        if (success) {
            toast({ title: "Link Copied!", description: "Analytics page link copied to clipboard." });
        } else {
    ...
    const filteredResponses = useMemo(() => {
        if (!analysisResults) return [];
        if (!filterText) return analysisResults.individual_responses;

        const lowerFilter = filterText.toLowerCase();
        return analysisResults.individual_responses.filter(r =>
            r.persona_name.toLowerCase().includes(lowerFilter) ||
            r.reasoning.toLowerCase().includes(lowerFilter) ||
            r.persona_id.toString().includes(lowerFilter)
        );
    }, [analysisResults, filterText]);
    ...
    const sentimentData = useMemo(() => {
        ...
        analysisResults.individual_responses.forEach(r => {
            const sentiment = r.responses.sentiment || 0;
    ...
    const metricsComparisonData = useMemo(() => {
        if (!analysisResults) return [];
        const { summary_statistics } = analysisResults;
        return [
            { name: 'Purchase Intent', value: summary_statistics.purchase_intent_avg || 0, fullMark: 10 },
            { name: 'Brand Trust', value: summary_statistics.trust_in_brand_avg || 0, fullMark: 10 },
            { name: 'Message Clarity', value: summary_statistics.message_clarity_avg || 0, fullMark: 10 },
        ];
    }, [analysisResults]);
                ...
                <div className="grid gap-8 md:grid-cols-2 mb-8">
                    ...
                    <Card className="border-0 shadow-2xl backdrop-blur-sm bg-white/90 dark:bg-gray-900/90">
                        <CardHeader>
                            ...
                            <CardDescription>Breakdown of cohort sentiment</CardDescription>
                        </CardHeader>
                    ...
                    <Card className="border-0 shadow-2xl backdrop-blur-sm bg-white/90 dark:bg-gray-900/90">
                        <CardHeader>
                            <CardTitle>Key Metrics Average</CardTitle>
                            <CardDescription>Performance across key indicators (0-10)</CardDescription>
                        </CardHeader>
                        <CardContent className="h-[300px]">
                            <ResponsiveContainer width="100%" height="100%">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                    {filteredResponses.map((response: IndividualResponseRow, index: number) => (
                                        <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                                            ...
                                            <td className="p-4 max-w-md align-top">
                                                <div
                                                    className={`text-sm text-gray-600 dark:text-gray-400 ${expandedRows.has(index) ? '' : 'line-clamp-2'} cursor-pointer`}
                                                    onClick={() => toggleRowExpansion(index)}
                                                >
                                                    {response.reasoning}
                                                </div>
                                                {response.reasoning.length > 100 && (
                                                    <button
                                                        onClick={() => toggleRowExpansion(index)}
                                            ...
                                            <td className="p-4 align-top">
                                                <Button variant="ghost" size="sm" onClick={() => copyToClipboard(response.reasoning)}>
